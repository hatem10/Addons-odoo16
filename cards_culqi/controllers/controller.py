...
class culqi_controller(http.Controller):
    ...
    def process_culqi_payment(self, **kw):
        ...
        try:
            ...
            _logger.warning(str('Error beetwen partner realationship'))
            pass
        ...
        phone_number = params['customer']['mobile'] if (params['customer']['mobile']) else params['customer']['phone']
        phone_number = re.findall('\.[0-9.]+', phone_number)
        
        _logger.warning("phone_number - address")
        _logger.warning(phone_number)
        ...
        if(str(res_charge['data']['outcome']['type'])=="venta_exitosa"):
            ...
            try:
                ...
                if(not setting):
                    setting = {}
                    setting['automatic_invoice'] = True
                setting['autoconfirm_payment'] = acquirer['autoconfirm_invoice']
